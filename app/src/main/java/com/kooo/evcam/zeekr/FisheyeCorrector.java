...
public final class FisheyeCorrector {
    ...
    public static Bitmap correctGrid(Bitmap source, int columns, int rows,
        ...
        // 合成流的每一路都是正方形（实测 1280x1280），拼起来也是正方形。
        // 不是正方形就说明这张图根本不是四宫格 —— 配置里的「排列」标记在座舱那几路上
        // 也是 true（StreamSpec.grid 的默认值），信它就会把一张 3840x2160 的座舱照片
        // 切成四块分别校正。所以这里认画面，不认标记。
        if (columns > 1 || rows > 1) {
            float ratio = (float) cellWidth / cellHeight;
            if (ratio < 0.96f || ratio > 1.04f) {
                AppLog.i(TAG, "这张图的格子是 " + cellWidth + "x" + cellHeight
                        + "，不是方的，按整张处理，不做校正");
                return source;
            }
        }
